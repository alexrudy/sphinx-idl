...
class IDLAutoBase(Directive):
    ...
    def handle_idl_object(self, obj):
        ...
        if obj.kind == 'pro':
            directive_line = ".. idl:pro:: pro {}, {}".format(obj.name, obj.signature)
        elif obj.kind == 'function':
            directive_line = ".. idl:function:: function {}, {}".format(obj.name, obj.signature)
...
class IDLAutoPath(IDLAutoBase):
    ...
    option_spec = {'encoding': directives.encoding, 'absolute': directives.flag, 'glob' : directives.unchanged, 'summary' : directives.flag }
